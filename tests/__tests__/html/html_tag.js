...
describe('div plugin', () => {
	...
	test('generates div tag with classes shorthand and class attribute', () => {
		...
		const expectedOutput = '<div class="anotherClass bordered myClass">This is the div content</div>'
		expect(textSynth.merge(input, {})).toBe(expectedOutput)
	})
	...
	test('generates div tag with classes shorthand using underscores and dashes', () => {
		...
		const expectedOutput = '<div class="another_class my-Class">This is the div content</div>'
		expect(textSynth.merge(input, {})).toBe(expectedOutput)
	})
	...
	test('generates div tag with classes shorthand and class attribute with unique results', () => {
		...
		const expectedOutput = '<div class="aDifferentClass anotherClass bordered myClass">This is the div content</div>'
		expect(textSynth.merge(input, {})).toBe(expectedOutput)
	})
